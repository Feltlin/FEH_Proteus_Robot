...
int main(){
    ...
    armServo.SetDegree(armServoAngle);

    double prongServoAngle = 0;
    prongServo.SetDegree(prongServoAngle);
    int frame = 0;
    ...
    while(true){
        ...
        if(text.button("++++", 0xff0000, 50, 50, 20)){
            ++armServoAngle;  
        }
        if(text.button("----", 0x0000ff, 150, 50, 20)){
            --armServoAngle;
        }
        armServoAngle = std::min(std::max(20., armServoAngle), 170.);
        text.display("Arm Servo Angle: " + std::to_string(armServoAngle), 0xffffff, 70, 20);
        if(frame == 0){
            armServo.SetDegree(armServoAngle);
        }

        if(text.button("++++", 0xff0000, 50, 120, 20)){
            ++prongServoAngle;  
        }
        if(text.button("----", 0x0000ff, 150, 120, 20)){
            --prongServoAngle;
        }
        prongServoAngle = std::min(std::max(20., prongServoAngle), 170.);
        text.display("Arm Servo Angle: " + std::to_string(prongServoAngle), 0xffffff, 70, 90);
        if(frame == 0){
            prongServo.SetDegree(prongServoAngle);
        }
